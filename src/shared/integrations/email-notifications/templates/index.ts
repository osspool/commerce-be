...
type TemplateFunction = (data: TemplateData) => TemplateResult;

/**
 * Format amount from smallest unit (paisa) to display (BDT)
 */
function formatAmount(amount: number, currency: string = 'BDT'): string {
 ...
 */
function formatItemsText(items: OrderItem[] = []): string {
  if (!items.length) return 'No items';
  return items
    .map((item) => `- ${item.productName} x${item.quantity} = ${formatAmount(item.price * item.quantity)}`)
    .join('\n');
}
 ...
 */
function formatAddress(address: DeliveryAddress | undefined): string {
...
const templates: Record<string, TemplateFunction> = {
  // ============ NEW ORDER (ADMIN) ============
  'purchase.created': (data: TemplateData): TemplateResult => ({
  ...
  'payment.verified': (data: TemplateData): TemplateResult => {
    // Admin notification
  ...
  // ============ REFUND PROCESSED ============
  'payment.refunded': (data: TemplateData): TemplateResult => {
    // Admin notification
    if (data.isAdminNotification) {
      return {
...
export function renderTemplate(templateName: string, data: Record<string, unknown>): TemplateResult {
  ...
  return template(data as TemplateData);
}
