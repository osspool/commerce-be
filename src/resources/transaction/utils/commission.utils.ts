/**
 * Commission CRUD Operations
 * Database operations for commission management
 */

import type { ClientSession } from 'mongoose';
import Transaction from '../transaction.model.js';

interface CommissionBreakdown {
  grossAmount: number;
  gatewayFeeAmount: number;
  netAmount: number;
}

interface CommissionObject extends CommissionBreakdown {
  rate: number;
  gatewayFeeRate: number;
  status: string;
  dueDate: Date;
}

interface CommissionStatusSummary {
  grossAmount: number;
  gatewayFees: number;
  netAmount: number;
  count: number;
}

interface CommissionSummary {
  pending: CommissionStatusSummary;
  due: CommissionStatusSummary;
  paid: CommissionStatusSummary;
  waived: CommissionStatusSummary;
  total: CommissionStatusSummary;
  [key: string]: CommissionStatusSummary;
}

interface GetDueOptions {
  overdue?: boolean;
  limit?: number;
}

/**
 * Calculate commission amounts
 */
export function calculateCommission(amount: number, commissionRate = 0, gatewayFeeRate = 0): CommissionBreakdown {
  const grossAmount = amount * commissionRate;
  const gatewayFeeAmount = amount * gatewayFeeRate;
  const netAmount = grossAmount - gatewayFeeAmount;

  return {
    grossAmount: Math.round(grossAmount * 100) / 100,
    gatewayFeeAmount: Math.round(gatewayFeeAmount * 100) / 100,
    netAmount: Math.round(netAmount * 100) / 100,
  };
}

/**
 * Build commission object for transaction
 */
export function buildCommissionObject(
  amount: number,
  rate: number,
  gatewayFeeRate = 0,
  dueDate: Date | null = null,
): CommissionObject {
  const calc = calculateCommission(amount, rate, gatewayFeeRate);

  return {
    rate,
    ...calc,
    gatewayFeeRate,
    status: 'pending',
    dueDate: dueDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days
  };
}

/**
 * Mark commission as due (after customer payment verified)
 */
export async function markCommissionDue(transactionId: string, session: ClientSession | null = null) {
  return Transaction.findByIdAndUpdate(
    transactionId,
    {
      $set: {
        'commission.status': 'due',
      },
    },
    { returnDocument: 'after', session },
  ).exec();
}

/**
 * Mark commission as paid by organization
 */
export async function markCommissionPaid(
  transactionId: string,
  paidBy: string,
  notes = '',
  session: ClientSession | null = null,
) {
  return Transaction.findByIdAndUpdate(
    transactionId,
    {
      $set: {
        'commission.status': 'paid',
        'commission.paidDate': new Date(),
        'commission.paidBy': paidBy,
        'commission.notes': notes,
      },
    },
    { returnDocument: 'after', session },
  ).exec();
}

/**
 * Waive commission (platform decides not to charge)
 */
export async function waiveCommission(transactionId: string, reason = '', session: ClientSession | null = null) {
  return Transaction.findByIdAndUpdate(
    transactionId,
    {
      $set: {
        'commission.status': 'waived',
        'commission.notes': reason,
      },
    },
    { returnDocument: 'after', session },
  ).exec();
}

/**
 * Get all due commissions for an organization
 */
export async function getDueCommissions(organizationId: string, options: GetDueOptions = {}) {
  const query: Record<string, unknown> = {
    organizationId,
    'commission.status': 'due',
  };

  // Filter by overdue
  if (options.overdue) {
    query['commission.dueDate'] = { $lt: new Date() };
  }

  return Transaction.find(query)
    .sort({ 'commission.dueDate': 1 })
    .select('amount commission category sourceModel sourceId date')
    .limit(options.limit || 100)
    .lean()
    .exec();
}

/**
 * Get commission summary for organization
 * Returns gross, gateway fees, and net amounts
 */
export async function getCommissionSummary(organizationId: string): Promise<CommissionSummary> {
  const result = await Transaction.aggregate([
    {
      $match: {
        organizationId,
        'commission.grossAmount': { $exists: true, $gt: 0 },
      },
    },
    {
      $group: {
        _id: '$commission.status',
        grossAmount: { $sum: '$commission.grossAmount' },
        gatewayFees: { $sum: '$commission.gatewayFeeAmount' },
        netAmount: { $sum: '$commission.netAmount' },
        count: { $sum: 1 },
      },
    },
  ]).exec();

  // Convert to object with breakdown
  const summary: CommissionSummary = {
    pending: { grossAmount: 0, gatewayFees: 0, netAmount: 0, count: 0 },
    due: { grossAmount: 0, gatewayFees: 0, netAmount: 0, count: 0 },
    paid: { grossAmount: 0, gatewayFees: 0, netAmount: 0, count: 0 },
    waived: { grossAmount: 0, gatewayFees: 0, netAmount: 0, count: 0 },
    total: { grossAmount: 0, gatewayFees: 0, netAmount: 0, count: 0 },
  };

  result.forEach(
    (item: { _id: string; grossAmount: number; gatewayFees: number; netAmount: number; count: number }) => {
      if (item._id) {
        summary[item._id] = {
          grossAmount: item.grossAmount,
          gatewayFees: item.gatewayFees,
          netAmount: item.netAmount,
          count: item.count,
        };
      }
      summary.total.grossAmount += item.grossAmount;
      summary.total.gatewayFees += item.gatewayFees;
      summary.total.netAmount += item.netAmount;
      summary.total.count += item.count;
    },
  );

  return summary;
}

export default {
  markCommissionDue,
  markCommissionPaid,
  waiveCommission,
  getDueCommissions,
  getCommissionSummary,
};
