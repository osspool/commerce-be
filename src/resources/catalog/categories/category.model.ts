...
// Virtuals
categorySchema.virtual('fullPath').get(function (this: CategoryDocument) {
  if (this.parent) {
...
categorySchema.virtual('isRoot').get(function (this: CategoryDocument) {
  return !this.parent;
});
...
categorySchema.pre('deleteOne', { document: true, query: false }, async function (this: CategoryDocument) {
  if (this.productCount > 0) {
    const error: ErrorWithStatusCode = new Error(
      `Cannot delete category "${this.name}": ${this.productCount} products still reference it`,
    );
