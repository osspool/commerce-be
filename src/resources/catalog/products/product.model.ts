...
export interface IStockProjectionVariant {
  sku?: string;
  quantity: number;
}

export interface IStockProjection {
  variants: IStockProjectionVariant[];
  syncedAt?: Date;
}
...
export interface IProduct {
  ...
  tags: string[];
  stats: IProductStats;
  stockProjection: IStockProjection;
 ...
 */
const variationAttributeSchema = new Schema<IVariationAttribute>(
  {
...
const statsSchema = new Schema<IProductStats>(
  {
...
const stockProjectionSchema = new Schema<IStockProjection>(
  {
    variants: [
      {
...
// POS/inventory listing: covers { isActive, deletedAt } filter + name sort
productSchema.index({ isActive: 1, deletedAt: 1, name: 1, _id: 1 });
...
// Virtuals
productSchema.virtual('isDiscountActive').get(function (this: ProductDocument) {
...
productSchema.virtual('currentPrice').get(function (this: ProductDocument) {
...
productSchema.virtual('featuredImage').get(function (this: ProductDocument) {
  if (!this.images?.length) return null;
  return this.images.find((img) => img.isFeatured) || this.images[0];
});

productSchema.virtual('totalSales').get(function (this: ProductDocument) {
...
productSchema.virtual('profitMarginPercent').get(function (this: ProductDocument) {
...
productSchema.virtual('isDeleted').get(function (this: ProductDocument) {
