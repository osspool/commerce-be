/**
 * Product Utilities
 * Helper functions for product operations
 */

import config from '#config/index.js';

interface UserWithRoles {
  role?: string | string[];
  [key: string]: unknown;
}

interface BranchStockVariant {
  costPrice?: number;
  [key: string]: unknown;
}

interface BranchStock {
  variants?: BranchStockVariant[];
  [key: string]: unknown;
}

interface ProductLike {
  costPrice?: number;
  profitMargin?: number;
  profitMarginPercent?: number;
  variants?: Array<{ costPrice?: number; [key: string]: unknown }>;
  branchStock?: BranchStock;
  matchedVariant?: { costPrice?: number; [key: string]: unknown } | null;
  [key: string]: unknown;
}

function normalizeRoles(userOrRoles: string | string[] | UserWithRoles | null | undefined): string[] {
  if (!userOrRoles) return [];
  if (Array.isArray(userOrRoles)) return userOrRoles.filter(Boolean);
  if (typeof userOrRoles === 'string') return [userOrRoles];
  const roles = (userOrRoles as UserWithRoles).role;
  if (Array.isArray(roles)) return roles.filter(Boolean);
  if (typeof roles === 'string') return [roles];
  return [];
}

export function canViewCostPrice(userOrRoles: string | string[] | UserWithRoles | null | undefined): boolean {
  const roles = normalizeRoles(userOrRoles);
  const allowed: readonly string[] = config.costPrice?.viewRoles || [];
  return roles.some((r) => allowed.includes(r));
}

export function canManageCostPrice(userOrRoles: string | string[] | UserWithRoles | null | undefined): boolean {
  const roles = normalizeRoles(userOrRoles);
  const allowed: readonly string[] = config.costPrice?.manageRoles || [];
  return roles.some((r) => allowed.includes(r));
}

/**
 * Filter cost price fields based on user role
 * Only admin and store-manager can view cost prices
 */
export function filterCostPriceByRole<T extends ProductLike | ProductLike[]>(
  data: T,
  userOrRoles: string | string[] | UserWithRoles | null | undefined,
): T {
  if (!data) return data;

  if (canViewCostPrice(userOrRoles)) {
    return data; // Return as-is for authorized roles
  }

  // Remove cost price from response for unauthorized roles
  if (Array.isArray(data)) {
    return data.map((item) => removeCostPriceFields(item)) as T;
  }

  return removeCostPriceFields(data as ProductLike) as T;
}

/**
 * Remove cost price fields from a product object
 */
function removeCostPriceFields(product: ProductLike): ProductLike {
  if (!product) return product;

  const filtered = { ...product };
  delete filtered.costPrice;
  delete filtered.profitMargin;
  delete filtered.profitMarginPercent;

  // Remove cost price from NEW variants structure
  if (filtered.variants) {
    filtered.variants = filtered.variants.map((variant) => {
      const { costPrice, ...rest } = variant;
      return rest;
    });
  }

  // POS responses may include branchStock.variants[].costPrice
  if (filtered.branchStock?.variants?.length) {
    filtered.branchStock = { ...filtered.branchStock };
    filtered.branchStock.variants = filtered.branchStock.variants?.map((v) => {
      if (!v || typeof v !== 'object') return v;
      const { costPrice, ...rest } = v;
      return rest;
    });
  }

  // Lookup responses may include a matchedVariant snapshot
  if (filtered.matchedVariant && typeof filtered.matchedVariant === 'object') {
    const { costPrice, ...rest } = filtered.matchedVariant;
    filtered.matchedVariant = rest;
  }

  return filtered;
}
