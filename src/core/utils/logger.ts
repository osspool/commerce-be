...
export const createFastifyLogger = (): Record<string, unknown> => ({
  logger: {
    level: process.env.LOG_LEVEL || (isDev ? 'debug' : 'info'),
    // Serialize errors properly
    serializers: {
      ...
      error: pino.stdSerializers.err,
      err: pino.stdSerializers.err,
      req(request: unknown): { id: string; method: string; url: string } {
      ...
      res(reply: unknown): { statusCode: number } {
        const res = reply as ReplyLike;
        return {
