...
- Transaction is created via background job for non-blocking checkout
- Job runs in-process (same API server) - no separate worker needed
- Retries automatically on failure
- Order response returns immediately; transaction created async
...
POS order VAT data automatically flows to the associated Transaction (created via background job) for finance/accounting reporting:

**Transaction Tax Fields (populated from POS Order):**
...
| `source` | `'pos'` | Identifies POS origin |
| `branch` | Order branch | Branch where sale occurred |
...
```json
{
  ...
  "metadata": {
    "branchCode": "DHK",
    "cashierId": "user_id",
    "vatInvoiceNumber": "INV-DHK-20240115-0001"
  }
}
```
...
- **Branches**: `GET /api/v1/branches`, `GET /api/v1/branches/default` — see [Branch API](../commerce/branch.md)
- **Platform Config**: `GET /api/v1/platform/config` — see [Platform API](../platform/platform.md)
