...
```javascript
{
  internalId: 1,            // Our internal area ID (save as areaId)
  name: 'Mohammadpur',      // Display name
  postCode: '1207',         // Postal code
  zoneId: 1,                // Zone ID for pricing
  providers: {              // ⚠️ IMPORTANT: Save as providerAreaIds for logistics API
    redx: 1,                // RedX-specific area ID
    pathao: 101             // Pathao-specific area ID
  }
}
```
...
```json
{
  ...
  "deliveryAddress": {
    ...
    "providerAreaIds": { "redx": 1 },     // Required for logistics API (from area.providers)
...
interface CreateOrderPayload {
  ...
  deliveryAddress: {
    ...
    providerAreaIds?: {               // Required for logistics API integration
      redx?: number;                  // From area.providers.redx (bd-areas)
      pathao?: number;                // From area.providers.pathao (bd-areas)
    };
