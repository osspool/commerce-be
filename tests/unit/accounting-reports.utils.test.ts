...
describe('parseDateParams', () => {
  ...
  it('parses custom range', () => {
    ...
    if (r.dateOption === 'custom') {
      expect(r.dateValue.startDate.getUTCFullYear()).toBe(2024);
      expect(r.dateValue.endDate.getUTCMonth()).toBe(2);
    }
  });

  // Contract lock: ledger's getDateRange('custom', value) destructures
  // { startDate, endDate } from value. If we ever rename these keys, the
  // ledger throws "Custom date range requires both startDate and endDate"
  // and every report endpoint with dateOption=custom 500s. Fail-fast at
  // unit-test time instead of waiting for a runtime report error.
  it('emits dateValue with the exact keys the ledger destructures (startDate/endDate)', () => {
    const r = parseDateParams({
      dateOption: 'custom',
      startDate: '2024-01-01',
      endDate: '2024-03-31',
    });
    if (r.dateOption !== 'custom') throw new Error('expected custom branch');
    expect(Object.keys(r.dateValue).sort()).toEqual(['endDate', 'startDate']);
    expect(r.dateValue.startDate).toBeInstanceOf(Date);
    expect(r.dateValue.endDate).toBeInstanceOf(Date);
  });
