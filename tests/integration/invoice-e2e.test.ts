...
describe('Custom Invoice Routes', () => {
  ...
  it('GET /payment-terms — returns MongoKit OffsetPaginationResponse<PaymentTerm>', async () => {
    const res = await server.inject({
      method: 'GET',
      url: `${API}/accounting/invoices/payment-terms`,
      headers: h(),
    });

    expect(res.statusCode).toBe(200);
    const body = parse(res.body);
    // Response is MongoKit's OffsetPaginationResult spread into the envelope —
    // NOT { success, data: [...] }. The SDK hook reads res.docs directly.
    expect(body.success).toBe(true);
