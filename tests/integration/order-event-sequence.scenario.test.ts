...
async function seedStock(qty: number): Promise<void> {
  ...
  const { seedStock: erpSeedStock } = await import('../helpers/erp-seed.js');
  await erpSeedStock(getFlowEngine(), env.orgId, sku, qty, 12000);
}
...
beforeEach(async () => {
  ...
  await Promise.all([
    db.collection('flow_stock_quants').deleteMany({ skuRef: sku }),
    db.collection('flow_stock_moves').deleteMany({ skuRef: sku }),
    db.collection('flow_stock_move_lines').deleteMany({ skuRef: sku }),
    db.collection('flow_stock_move_groups').deleteMany({}),
    db.collection('flow_cost_layers').deleteMany({ skuRef: sku }),
    db.collection('flow_reservations').deleteMany({ skuRef: sku }),
    db.collection('flow_stock_events').deleteMany({ skuRef: sku }),
...
describe('Order saga compensation — all-or-nothing contract', () => {
  it('invalid offerId → 4xx, no order persisted, no reservation, no event', async () => {
    ...
    expect(await db.collection('reservations').countDocuments({ skuRef: sku })).toBe(0);
