...
describe('Resource smoke', () => {
  ...
  it('creates a category for admin', async () => {
    ...
    const res = await server.inject({
      ...
      headers: authHeaders(adminToken),
      // Category's Zod schema now requires slug + displayOrder + isActive
      // alongside name (was just `name` pre-2026-04). Keep the payload
      // minimal-but-complete so this stays a smoke test.
      payload: {
        name: `Migration Category ${ts}`,
        slug: `migration-category-${ts}`,
        displayOrder: 0,
        isActive: true,
      },
    });
...
describe('Arc exports', () => {
  it('exposes core and testing APIs', async () => {
    ...
    // Arc 2.11 testing surface rewrite: `request` + `createTestAuth` went
    // away; `createTestApp` + `expectArc` + `createHttpTestHarness` cover
    // the same use cases.
    expect(typeof testing.createTestApp).toBe('function');
    expect(typeof testing.expectArc).toBe('function');
    expect(typeof testing.createHttpTestHarness).toBe('function');
  });
});
