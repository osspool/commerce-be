...
describe('GET /pos/products — canonical OffsetPaginationResult envelope', () => {
  it('returns { method, docs, page, limit, total, pages, hasNext, hasPrev } on first page', async () => {
    const res = await env.server.inject({
      ...
      headers: { ...env.auth.getHeaders('admin'), 'x-organization-id': env.orgId },
    });
...
describe('GET /pos/products — stock enrichment', () => {
  ...
  it('inStockOnly trims the page but `total` still reflects pre-filter count (documented caveat)', async () => {
    const res = await env.server.inject({
      method: 'GET',
    ...
    // ...and `docs.length` ≤ number of stocked products.
    expect(body.docs.length).toBe(STOCKED_PRODUCT_IDS.length);

    // Caveat: `total`/`pages` reflect the catalog-level filter (all 35
    // active products), NOT the post-enrichment trim. UI consumers must
    // not assume `total === docs.length` when inStockOnly is on. Push
    // stock into the aggregate via LookupBuilder if this becomes a UX
    // problem — see pos.utils.ts header comment.
    expect(body.total).toBe(35);
  });
