...
beforeAll(async () => {
  ...
  auth = createBetterAuthProvider({
    tokens: { admin: token }, orgId: ctx.orgId, adminRole: 'admin',
  });
  ...
  const { setupBranch } = await import('../helpers/erp-seed.js');
...
describe('PATCH /users/me — self-update', () => {
  ...
  it('PATCH /me with name round-trips through the BA-shared user collection', async () => {
    ...
    const patch = await server.inject({
      ...
      headers: auth.getHeaders('admin'), payload: { name: newName },
    });
