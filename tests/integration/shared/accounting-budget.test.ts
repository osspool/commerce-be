...
describe('Enterprise Mode Gate', () => {
  it('CRUD routes registered (not 404)', async () => {
    const res = await server.inject({ method: 'GET', url: `${API}/accounting/budgets`, headers: auth.as('admin').headers });
  ...
  it('summary route registered', async () => {
    const res = await server.inject({ method: 'GET', url: `${API}/accounting/budgets/summary`, headers: auth.as('admin').headers });
    expect(res.statusCode).not.toBe(404);
  });
...
describe('Arc CRUD', () => {
  ...
  it('GET /:id — get single budget', async () => {
    const res = await server.inject({ method: 'GET', url: `${API}/accounting/budgets/${budgetId}`, headers: auth.as('admin').headers });
    expect([200, 403]).toContain(res.statusCode);
