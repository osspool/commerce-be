...
beforeAll(async () => {
  ...
  const email = `rma-admin-${ts}@test.com`;
    ...
    const __testApp = await createApplication({ resources: preloadedResources as any });
...
describe('Return / RMA Lifecycle', () => {
  ...
  describe('Create Return', () => {
    ...
    it('rejects return when window expired', async () => {
      const db = mongoose.connection.db!;
      ...
      await db.collection('orders').insertOne({
        ...
        // `shipping.deliveredAt` is the first fallback the return service reads
        // before walking to OrderFulfillment docs (return.service.ts:201).
        shipping: { deliveredAt },
        paymentState: { amount: 100, status: 'verified', transactions: [] },
        createdAt: new Date(Date.now() - 35 * 24 * 60 * 60 * 1000),
        updatedAt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
      });
      // Belt-and-braces: also seed a delivered OrderFulfillment so resolveDeliveredAt
      // succeeds via the canonical fulfillment path even if the Order schema
      // strips `shipping.deliveredAt` on load.
