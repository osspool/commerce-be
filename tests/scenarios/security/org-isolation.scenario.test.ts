...
import { SetupBetterAuthTestAppResult } from '@classytic/arc/testing';
...
let authA: TestAuthProvider;
let adminAUserId: string;

let ctxB;
let authB: TestAuthProvider;
let orgB: string;

const hA = (): Record<string, string> => authA.as('admin').headers;
const hB = (): Record<string, string> => authB.as('admin').headers;
...
beforeAll(async () => {
  ...
  const { setupBetterAuthTestApp, createBetterAuthProvider } = await import('@classytic/arc/testing');
  const { getAuth } = await import('#resources/auth/auth.config.js');
  const ts = Date.now();
    const __testApp = await Promise.resolve(server);
ctxB = await setupBetterAuthTestApp({
    app: __testApp,
  ...
  authB = createBetterAuthProvider({ defaultOrgId: orgB });
  authB.register('admin', { token: token });
}, 180_000);
...
describe('Org isolation — header forgery', () => {
  ...
  it('admin A with no x-organization-id → 401/403 on tenant-scoped route', async () => {
    // Arc 2.11's TestAuthProvider.headers is frozen — spread into a mutable
    // copy before stripping the tenant header.
    const { 'x-organization-id': _omit, ...headers } = hA();
