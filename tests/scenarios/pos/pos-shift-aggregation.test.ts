...
function headers() {
  return { ...env.auth.as('admin').headers, 'x-organization-id': env.orgId };
}
...
describe('applyShiftAggregation', () => {
  ...
  it('falls back to gateway field when payments[] is empty', async () => {
    ...
    const payload = buildOrderPayload({
      shiftId,
      grandTotalMinor: 150000, // 1500 BDT in paisa
      gateway: 'cash',
    });
    await applyShiftAggregation(payload);
    const shift = await PosShift.findById(shiftId).lean();
    const cashRow = shift?.paymentBreakdown?.find((r) => r.method === 'cash');
    // Paisa end-to-end — `salesAmount` is the gross figure in minor units,
    // not converted back to major units (the legacy /100 division was
    // removed alongside the date-based aggregator).
    expect(cashRow?.salesAmount).toBe(150_000);
  });
