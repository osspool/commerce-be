...
describe('VAT Purchase-Sale Cycle', () => {
  ...
  it('POST /fulfillments/for-order/:orderNumber + deliver — stock decrement', async () => {
    ...
    // Physical FSM: pending → picking → packed → shipped → delivered. Walk
    // through each transition — `ship` decrements stock; `deliver` posts
    // accounting:order.fulfilled. Transition uses fulfillmentNumber, not _id.
    for (const action of ['pick', 'pack', 'ship', 'deliver']) {
      ...
      if (res.statusCode !== 200) console.log(`${action}:`, res.statusCode, res.body);
      expect(res.statusCode).toBe(200);
    }
  });
