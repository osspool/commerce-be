...
describe('Outsider / Regular User Restrictions', () => {
  ...
  it('regular user without branch membership cannot list orders (orgScoped preset)', async () => {
    // orgScoped preset enforces branch membership — an outsider spoofing
    // x-organization-id for a branch they're not a member of is rejected.
    const res = await app.inject({
      method: 'GET',
      url: '/api/v1/orders',
      headers: authHeaders(outsiderToken, branchAId),
