...
let otherCtx;
let otherAuth: TestAuthProvider;

const h = (): Record<string, string> => auth.as('admin').headers;
const h2 = (): Record<string, string> => otherAuth.as('admin').headers;
...
otherCtx = await setupBetterAuthTestApp({
    app: __testApp,
  ...
  otherAuth = createBetterAuthProvider({ defaultOrgId: otherCtx.orgId });
  otherAuth.register('admin', { token: token });
}, 120_000);
...
describe('CRM HTTP e2e — lead → converted → won via Arc actions', () => {
  ...
  it('qualifies the lead via POST /crm/leads/:id/action', async () => {
    const res = await server.inject({
      ...
      headers: h(),
      // qualify action uses an empty schema — no extras allowed.
