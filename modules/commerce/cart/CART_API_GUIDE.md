...
## Request Shapes
- Add item (`POST /items`): `{ productId: string (req), quantity: number>=1 (req), variations?: [{ name, option: { value } }] }`
...
- Status `200`.
- Body: `{ success: true, data: { _id, user, items: [{ _id, product:{ name, slug, images, variations, discount, basePrice }, variations, quantity }], createdAt, updatedAt } }`
