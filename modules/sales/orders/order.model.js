...
const SHIPPING_PROVIDERS = ['redx', 'pathao', 'steadfast', 'paperfly', 'sundarban', 'sa_paribahan', 'dhl', 'fedex', 'manual', 'other'];
...
const shippingHistorySchema = new Schema({
  ...
  raw: Schema.Types.Mixed, // Raw provider response for debugging
}, { _id: false });

const shippingChargesSchema = new Schema({
  deliveryCharge: { type: Number, default: 0 },
  codCharge: { type: Number, default: 0 },
  totalCharge: { type: Number, default: 0 },
}, { _id: false });

const shippingPickupSchema = new Schema({
  storeId: Number,
  storeName: String,
  scheduledAt: Date,
}, { _id: false });

const shippingCashCollectionSchema = new Schema({
  amount: { type: Number, default: 0 },
...
/**
 * Shipping Schema (Consolidated - replaces separate Shipment model)
 *
 * Contains all shipment tracking data for logistics providers.
 * Indexed by trackingNumber for fast webhook lookups.
 */
const shippingSchema = new Schema({
  provider: { type: String, enum: SHIPPING_PROVIDERS },
  status: { type: String, enum: SHIPPING_STATUSES, default: 'pending' },
  trackingNumber: String,
  providerOrderId: String, // Provider's internal ID if different from trackingNumber
...
orderSchema.index({ 'shipping.trackingNumber': 1 }, { sparse: true }); // Webhook lookups
